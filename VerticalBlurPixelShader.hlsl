...
float4 main(TargetCoords input) : SV_TARGET
{
	...
	for (int k = -blurDistance; k <= blurDistance; k++) {
		color += horizBlurTexture.Sample(blurSampler, input.uv + float2(0.0f, texelSize * k)) * weights[abs(clamp(k, 0, MAX_BLUR_PIXELS))];	//how to use diff weights -- need to calc weights depending on blurDistance
	}
